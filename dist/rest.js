...
function _applyCors(app, req, res) {
    const corsOptions = app.get("rest.cors");
    if (!corsOptions) {
    ...
    res.setHeader("Access-Control-Allow-Methods", corsOptions.methods ? (Array.isArray(corsOptions.methods) ? corsOptions.methods.join(",") : corsOptions.methods) : "GET,POST,PUT,PATCH,DELETE,OPTIONS");
    res.setHeader("Access-Control-Allow-Headers", corsOptions.allowedHeaders ? (Array.isArray(corsOptions.allowedHeaders) ? corsOptions.allowedHeaders.join(",") : corsOptions.allowedHeaders) : "Content-Type,Authorization");
    if (corsOptions.credentials) {
        res.setHeader("Access-Control-Allow-Credentials", "true");
    }
    if (corsOptions.exposedHeaders) {
        res.setHeader("Access-Control-Expose-Headers", Array.isArray(corsOptions.exposedHeaders) ? corsOptions.exposedHeaders.join(",") : corsOptions.exposedHeaders);
    }
...
async function _parseBody(app, req) {
    return new Promise((resolve, reject) => {
        const contentType = req.headers["content-type"] || "";
        const bodyParserOptions = app.get("rest.bodyParser") || {};
        ...
        req.on("data", (chunk) => {
            ...
            if (contentType.startsWith("application/json") && bodyParserOptions.json) {
                bodyLimit = parseSizeToBytes(bodyParserOptions.json?.limit || '1mb');
            }
            else if (contentType.startsWith("application/x-www-form-urlencoded") && bodyParserOptions.urlencoded) {
                bodyLimit = parseSizeToBytes(bodyParserOptions.urlencoded?.limit || '1mb');
            }
            else if (contentType.startsWith("text/") && bodyParserOptions.text) {
                bodyLimit = parseSizeToBytes(bodyParserOptions.text?.limit || '1mb');
            }
            else if (bodyParserOptions.raw) {
                bodyLimit = parseSizeToBytes(bodyParserOptions.raw?.limit || '1mb');
            }
            else {
        ...
        req.on("end", () => {
            ...
            try {
                if (contentType.startsWith("application/json") && bodyParserOptions.json) {
                    const jsonOptions = bodyParserOptions.json || {};
                    ...
                    const urlencodedOptions = bodyParserOptions.urlencoded || {};
                    ...
                    const textOptions = bodyParserOptions.text || {};
                    resolve(buffer.toString((textOptions.defaultCharset || 'utf8')));
                }
                else if (bodyParserOptions.raw) {
                    resolve(buffer);
                }
...
async function _compressResponse(app, req, // Added req to access accept-encoding
res, data, headers = {}) {
    const compressionOptions = app.get("rest.compression");
    ...
    if (typeof data === 'string' || Buffer.isBuffer(data)) {
        ...
        try {
            ...
            if (objectPrototypeToJSONExists) {
                Object.prototype.toJSON = originalObjectPrototypeToJSON;
...
async function _handleRequest(app, req, res) {
    ...
    try {
        ...
        const route = findRoute(app.getRouter(), method, requestPath); // Use app.getRouter()
        if (!route || !route.data) {
        ...
        if (["POST", "PUT", "PATCH"].includes(method.toUpperCase())) {
            ...
            try {
                ...
                if (error instanceof ScorpionError) {
                    ...
                    _sendResponse(res, 500, { error: 'Internal Server Error', details: error.message, path: requestPath });
                }
                // app.currentRequest = undefined;
                // app.currentResponse = undefined;
                return;
...
export function startRestServer(app, port, host) {
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            server.listen(port, host, () => {
                console.log(`[Scorpion REST] Server listening on http://${host}:${server.address()?.port || port}`);
